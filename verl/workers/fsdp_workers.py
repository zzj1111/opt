...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        self.use_orig_params = self.config.actor.fsdp_config.get("use_orig_params", False)  ###modified for opt project
    ...
    def _build_model_optimizer(
        ...
        # Freeze layers BEFORE FSDP init so each layer's FlatParameter is created
        # with the correct requires_grad flag — no need for use_orig_params=True.
        freeze_n_pre = OmegaConf.select(self.config, "actor.freeze_except_last_n_layers")
        train_layer_ids_cfg_pre = OmegaConf.select(self.config, "actor.train_layer_ids")
        if hasattr(actor_module, "model") and hasattr(actor_module.model, "layers"):
            total_layers_pre = len(actor_module.model.layers)
        else:
            total_layers_pre = None

        if total_layers_pre is not None and (train_layer_ids_cfg_pre is not None or
                (freeze_n_pre is not None and int(freeze_n_pre) > 0)):
        ...
        # >>>>>>>>>>>>>>>>>>>>>>
        # Sparse training: only train K randomly selected params within trainable layers
        ...
        if sparse_k is not None and int(sparse_k) > 0:
            ...
            if self.rank == 0:
                print(f"[sparse_train] Randomly selecting {sparse_k} params to train within trainable layers")

            # Collect all trainable params (requires_grad=True)
            trainable_params = []
            for name, p in actor_module_fsdp.named_parameters():
            ...
            # Distribute K slots across params proportionally to their size
            remaining_k = actual_k
            param_k_alloc = []
            for idx, (name, p) in enumerate(trainable_params):
                if idx == len(trainable_params) - 1:
                    k_i = remaining_k  # last param gets the remainder
                else:
                    k_i = int(actual_k * p.numel() / total_trainable)
                    k_i = min(k_i, p.numel())
                param_k_alloc.append(k_i)
                remaining_k -= k_i

            # Create random masks: True = trainable, False = frozen
            # Use fixed seed so all ranks get the same mask
            rng = torch.Generator(device="cpu")
            rng.manual_seed(42)
            for (name, p), k_i in zip(trainable_params, param_k_alloc):
                with torch.no_grad():
                    # Random permutation to select k_i indices

...
#     """
#     针对 FSDP + H200 优化的全能分析函数。
#     返回一个包含全局指标和最后一层详细指标的字典。
#     """
#     stats = {
#         "global": {},       # 全局标量 (Global Norms)
#         "last_layer": {},   # 最后一层详细数据 (Vectors/Lists)
#     }
    
#     # ------------------------------------------------------
#     # A. 辅助: 计算 Norm
#     # ------------------------------------------------------
#     def calc_norm(t, dim=None):
#         if t is None: return 0.0
#         if rms_norm:
#             if dim is None: return (t.norm().pow(2) / t.numel()).sqrt()
#             else: return (t.norm(dim=dim).pow(2) / t.size(dim)).sqrt()
#         else:
#             return t.norm(dim=dim)

#     # ------------------------------------------------------
#     # B. 全局分析 (Global Analysis) - 高效 AllReduce
#     # ------------------------------------------------------
#     # 初始化累加器
#     total_p_sq = torch.tensor(0.0, device=torch.cuda.current_device())
#     total_g_sq = torch.tensor(0.0, device=torch.cuda.current_device())
#     total_numel = torch.tensor(0.0, device=torch.cuda.current_device())
    
#     # 遍历本地分片计算
#     for p in model.parameters():
#         total_p_sq += p.norm().pow(2)
#         total_numel += p.numel()
#         if p.grad is not None:
#             total_g_sq += p.grad.norm().pow(2)
            
#     # 全局规约
#     # [ParamSq, GradSq, Numel]
#     vec = torch.stack([total_p_sq, total_g_sq, total_numel])
#     dist.all_reduce(vec, op=dist.ReduceOp.SUM)
    
#     # 计算最终全局范数
#     g_p_sq, g_g_sq, g_num = vec[0], vec[1], vec[2]
    
#     if rms_norm:
#         stats["global"]["param_norm"] = (g_p_sq / g_num).sqrt().item()
#         stats["global"]["grad_norm"]  = (g_g_sq / g_num).sqrt().item()
#     else:
#         stats["global"]["param_norm"] = g_p_sq.sqrt().item()
#         stats["global"]["grad_norm"]  = g_g_sq.sqrt().item()

#     # ------------------------------------------------------
#     # C. 最后一层深度分析 (Last Layer Analysis)
#     # ------------------------------------------------------
#     # 1. 定位 lm_head
#     lm_head = None
...
#         # 2. 获取完整权重和梯度 (显存足够，直接 Summon)
#         with FSDP.summon_full_params(model, with_grads=True):
#             w = lm_head.weight
#             g = lm_head.weight.grad
            
#             # 记录维度信息
#             stats["last_layer"]["shape"] = list(w.shape)
            
#             # 计算 Per-Token (dim=1) 的范数 -> List
#             # 只取前 10 个和 均值/最大值 以保持日志简洁，或者根据需要记录全部
#             w_norm_cls = calc_norm(w, dim=1)
#             stats["last_layer"]["w_norm_mean"] = w_norm_cls.mean().item()
#             stats["last_layer"]["w_norm_top5"] = w_norm_cls[:5].tolist()
            
#             if g is not None:
#                 g_norm_cls = calc_norm(g, dim=1)
#                 stats["last_layer"]["g_norm_mean"] = g_norm_cls.mean().item()
#                 stats["last_layer"]["g_norm_top5"] = g_norm_cls[:5].tolist()
                
#         # 3. 获取完整 Momentum 和 Eff LR (需要手动 Gather)
#         # 只有在 lm_head 的权重在 optimizer 里时才执行
#         # 注意: FSDP use_orig_params=True 时 key 是 parameter 对象
#         # 我们用 lm_head.weight (它是分片的) 去查 optimizer
        
#         # 必须在 summon 上下文之外获取分片引用
#         shard_w = lm_head.weight 
#         if shard_w in optimizer.state:
#             st = optimizer.state[shard_w]
#             if "exp_avg" in st and "exp_avg_sq" in st:
#                 # Gather Momentum
#                 local_mom = st["exp_avg"]
#                 local_var = st["exp_avg_sq"]
                
#                 # 简单处理：AllGather 拼起来
#                 ws = dist.get_world_size()
#                 m_list = [torch.zeros_like(local_mom) for _ in range(ws)]
#                 v_list = [torch.zeros_like(local_var) for _ in range(ws)]
#                 dist.all_gather(m_list, local_mom)
#                 dist.all_gather(v_list, local_var)
                
#                 full_mom = torch.cat(m_list, dim=0) # 假设按 dim 0 切分
#                 full_var = torch.cat(v_list, dim=0)
                
#                 # 如果形状不对(padding)，进行截断
#                 # 需要知道真实形状，这里假设 4B 模型是对齐的，或者忽略末尾 padding
#                 # 为了严谨，可以用 stats["last_layer"]["shape"] 来 view
#                 if "shape" in stats["last_layer"]:
#                     real_numel = stats["last_layer"]["shape"][0] * stats["last_layer"]["shape"][1]
#                     if full_mom.numel() > real_numel:
#                         full_mom = full_mom.flatten()[:real_numel].view(stats["last_layer"]["shape"])
#                         full_var = full_var.flatten()[:real_numel].view(stats["last_layer"]["shape"])
#                     else:
#                         full_mom = full_mom.view(stats["last_layer"]["shape"])
#                         full_var = full_var.view(stats["last_layer"]["shape"])

#                 # 计算 Momentum Norm
#                 m_norm_cls = calc_norm(full_mom, dim=1)
#                 stats["last_layer"]["mom_norm_mean"] = m_norm_cls.mean().item()
#                 stats["last_layer"]["mom_norm_top5"] = m_norm_cls[:5].tolist()
                
#                 # 计算 Effective LR
#                 # Formula: lr / (sqrt(var) + eps) * correction
#                 pg = optimizer.param_groups[0]
#                 lr, eps = pg['lr'], pg['eps']
#                 full_eff_lr = lr / (full_var.sqrt() + eps)
#                 eff_lr_cls = full_eff_lr.mean(dim=1)
                
#                 stats["last_layer"]["eff_lr_mean"] = eff_lr_cls.mean().item()
#                 stats["last_layer"]["eff_lr_top5"] = eff_lr_cls[:5].tolist()

#                 # 释放显存
#                 del m_list, v_list, full_mom, full_var, full_eff_lr
...
#     # 2. 准备全局累加器 (放在 GPU 上进行高效 reduce)
#     # [Param_Sq, Grad_Sq, Mom_Sq, EffLR_Sum, Numel]
#     accum = torch.zeros(5, device=torch.cuda.current_device())

#     for p in model.parameters():
#         # A. 基础属性
#         p_sq = p.norm().pow(2)
#         g_sq = p.grad.norm().pow(2) if p.grad is not None else torch.tensor(0.0, device=p.device)
#         numel = p.numel()
        
#         # B. 优化器状态属性 (Momentum & EffLR)
#         m_sq = torch.tensor(0.0, device=p.device)
#         elr_sum = torch.tensor(0.0, device=p.device)
        
#         if p in optimizer.state:
#             state = optimizer.state[p]
#             if "exp_avg" in state and "exp_avg_sq" in state:
#                 step = state.get("step", torch.tensor(1.0)).item()
#                 bias_c1 = 1 - beta1 ** step
#                 bias_c2 = 1 - beta2 ** step
                
#                 # 动量平方和
#                 m_sq = state["exp_avg"].norm().pow(2)
                
#                 # 有效学习率本地总和: lr_eff = lr / bias_c1 / (sqrt(v/bias_c2) + eps)
#                 # 我们对分片内的每个元素求和，后续除以全局 numel 得到均值
...
#     g_p_sq, g_g_sq, g_m_sq, g_elr_sum, g_num = accum.tolist()

#     # 4. 计算全局指标
#     if rms_norm:
#         stats["global"]["param_norm"] = (g_p_sq / g_num)**0.5
#         stats["global"]["grad_norm"]  = (g_g_sq / g_num)**0.5
#         stats["global"]["mom_norm"]   = (g_m_sq / g_num)**0.5
#     else:
#         stats["global"]["param_norm"] = g_p_sq**0.5
#         stats["global"]["grad_norm"]  = g_g_sq**0.5
#         stats["global"]["mom_norm"]   = g_m_sq**0.5
    
#     stats["global"]["eff_lr_mean"] = g_elr_sum / g_num
...
#         # Summon 权重和梯度进行分析
#         with FSDP.summon_full_params(model, with_grads=True):
#             w, g = lm_head.weight, lm_head.weight.grad
#             stats["last_layer"]["shape"] = list(w.shape)
#             stats["last_layer"]["param_cls_norm"] = w.norm(dim=1).tolist()
#             if g is not None:
#                 stats["last_layer"]["grad_cls_norm"] = g.norm(dim=1).tolist()

#         # 手动 Gather 优化器状态 (H200 显存充足)
#         shard_w = lm_head.weight
#         if shard_w in optimizer.state:
#             st = optimizer.state[shard_w]
#             step = st.get("step", torch.tensor(1.0)).item()
            
#             # 辅助 Gather 函数
#             def gather_and_view(local_tensor):
#                 ws = dist.get_world_size()
#                 buffer = [torch.zeros_like(local_tensor) for _ in range(ws)]
#                 dist.all_gather(buffer, local_tensor)
#                 full = torch.cat(buffer, dim=0)
#                 # 截断 padding 并 reshape
#                 real_numel = stats["last_layer"]["shape"][0] * stats["last_layer"]["shape"][1]
#                 return full.flatten()[:real_numel].view(stats["last_layer"]["shape"])

#             full_m = gather_and_view(st["exp_avg"])
#             full_v = gather_and_view(st["exp_avg_sq"])
            
#             # 计算 Per-token 动量范数和有效学习率
#             stats["last_layer"]["mom_cls_norm"] = full_m.norm(dim=1).tolist()
            
#             bias_c1, bias_c2 = 1 - beta1**step, 1 - beta2**step
#             full_elr = lr / bias_c1 / ((full_v.sqrt() / (bias_c2**0.5)).add_(eps))
#             stats["last_layer"]["eff_lr_cls_mean"] = full_elr.mean(dim=1).tolist()
...
#     # 2. 全局标量计算 (这一步不涉及顺序问题，之前的逻辑是安全的)
#     accum = torch.zeros(5, device=torch.cuda.current_device())
#     for p in model.parameters():
#         accum[0] += p.norm().pow(2)
#         accum[4] += p.numel()
#         if p.grad is not None:
#             accum[1] += p.grad.norm().pow(2)
#         if p in optimizer.state:
#             st = optimizer.state[p]
#             if "exp_avg" in st:
#                 accum[2] += st["exp_avg"].norm().pow(2)
#                 step = st.get("step", torch.tensor(1.0, device=p.device)).float().mean()
#                 bias_c2 = 1 - beta2 ** step
#                 denom = (st["exp_avg_sq"].sqrt() / (bias_c2 ** 0.5)).add_(eps)
#                 accum[3] += (lr / (1 - beta1 ** step) / denom).sum()

#     dist.all_reduce(accum, op=dist.ReduceOp.SUM)
#     stats["global"]["param_norm"] = (accum[0] / (accum[4] if rms_norm else 1.0))**0.5
#     stats["global"]["grad_norm"] = (accum[1] / (accum[4] if rms_norm else 1.0))**0.5
#     stats["global"]["mom_norm"] = (accum[2] / (accum[4] if rms_norm else 1.0))**0.5
#     stats["global"]["eff_lr_mean"] = (accum[3] / accum[4]).item()
...
#         # --- 核心黑科技：借道还原 ---
#         # 我们把本地的 momentum 分片暂时存入 grad 分片中
#         # 然后调用 summon_full_params(with_grads=True)
#         # FSDP 会帮我们处理所有的 Padding、对齐和顺序映射，还原出真正的全量矩阵
        
#         orig_grad = shard_w.grad  # 备份原始梯度分片
        
#         def get_full_matrix_via_fsdp(state_key):
#             if shard_w not in optimizer.state or state_key not in optimizer.state[shard_w]:
#                 return None
            
#             # 将要分析的状态（如 exp_avg）塞进 grad
#             shard_w.grad = optimizer.state[shard_w][state_key].to(shard_w.dtype)
            
#             full_matrix = None
#             # 调用 FSDP 官方还原逻辑：这是唯一保证顺序正确的方法
#             with FSDP.summon_full_params(model, with_grads=True):
#                 if rank == 0:
#                     # 此时的 lm_head.weight.grad 已经被 FSDP 还原成了完整的、有序的状态矩阵
...
#                 stats["last_layer"]["param_cls_norm"] = lm_head.weight.norm(dim=1).cpu().tolist()
#                 if lm_head.weight.grad is not None:
#                     stats["last_layer"]["grad_cls_norm"] = lm_head.weight.grad.norm(dim=1).cpu().tolist()

#         # 2. 还原有序的动量 (exp_avg)
#         full_mom = get_full_matrix_via_fsdp("exp_avg")
#         if rank == 0 and full_mom is not None:
#             stats["last_layer"]["mom_cls_norm"] = full_mom.norm(dim=1).cpu().tolist()
#             del full_mom

#         # 3. 还原有序的平方动量 (exp_avg_sq) 并计算 EffLR
#         full_v = get_full_matrix_via_fsdp("exp_avg_sq")
#         if rank == 0 and full_v is not None:
#             st = optimizer.state[shard_w]
#             step = st.get("step", torch.tensor(1.0)).float().mean().item()
#             bias_c1, bias_c2 = 1 - beta1**step, 1 - beta2**step
            
#             # 计算有序的 Per-token Effective LR
#             full_v.sqrt_().div_(bias_c2**0.5).add_(eps)
#             full_elr = (lr / bias_c1) / full_v

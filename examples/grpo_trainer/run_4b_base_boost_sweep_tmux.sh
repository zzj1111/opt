#!/bin/bash
# ==============================================================================
# Qwen3-4B-Base boost-LR sweep (8 GPUs, sequential 10 experiments)
# ==============================================================================
#
...
#   2. top1  boost  (16)                           base_lr=1e-6  boost_lr=2e-6
#   3. top10 boost  (16/14/19/22/17/15/12/18/20/10) base_lr=1e-6  boost_lr=2e-6
#
# Phase B — Top-N train-only (freeze the rest):
#   4. top5  only   (16/14/19/22/17)               lr=2e-6
#   5. top10 only   (16/14/19/22/17/15/12/18/20/10) lr=2e-6
#
# Phase C — Worst-N boost ablation (paired with Phase A; expects degradation):
#   6. worst5  boost  (2/3/1/30/32)                       base_lr=1e-6  boost_lr=2e-6
#   7. worst1  boost  (2)                                 base_lr=1e-6  boost_lr=2e-6
#   8. worst10 boost  (2/3/1/30/32/29/27/33/4/28)         base_lr=1e-6  boost_lr=2e-6
#
# Phase D — Worst-N train-only ablation (paired with Phase B):
#   9.  worst5  only   (2/3/1/30/32)                      lr=2e-6
#   10. worst10 only   (2/3/1/30/32/29/27/33/4/28)        lr=2e-6
#
# Layer ranking by single-layer math_avg (no AIME, Qwen3-4B-Base 8K eval).
# Source: /home/zha00175/csvdoc/wandb_opt_rl_eval_analysis_final.xlsx
#   Full RL math_avg = 0.6365  (highest single layer L16 = 0.6432)
#   Sweet spot: L10-L22 (mid-band).  Worst: L1-L4 + L27-L33 (tails).
#
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="boost_sweep_4b_$(date +%m%d_%H%M)"

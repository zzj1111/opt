#!/bin/bash
# ==============================================================================
# 4 Experiments: Qwen3-1.7B RL training with NuminaMath-CoT (with tmux)
# ==============================================================================
#
# Experiments:
#   1. Full RL NuminaMath-CoT,           LR=1e-6
#   2. Layer 14 only NuminaMath-CoT,     LR=5e-6
#   3. Layer 0 only NuminaMath-CoT,      LR=5e-5
#   4. Layer 27 only NuminaMath-CoT,     LR=5e-5
#
...
#
# Usage:
#   bash run_numina_cot_4exp_tmux.sh                          # Run all 4
#   bash run_numina_cot_4exp_tmux.sh --gpus 0,1,2,3,4,5,6,7  # Use specific GPUs
#   bash run_numina_cot_4exp_tmux.sh --skip 2                 # Skip first 2
#   bash run_numina_cot_4exp_tmux.sh --only 1,3               # Run only 1 and 3
#   bash run_numina_cot_4exp_tmux.sh --no-tmux                # Skip tmux auto-launch
#
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="numina_4exp_$(date +%m%d_%H%M)"
    ...
    tmux new-session -d -s "$TMUX_SESSION" \
        "source $CONDA_INIT && \
         ...
         cd $PROJ_DIR && \
         bash $SCRIPT_DIR/run_numina_cot_4exp_tmux.sh $FULL_ARGS; \
         exec bash"
...
# --- Exp 2: Layer 14 only NuminaMath-CoT, LR=5e-6 ---
EXP2_NAME="${DATE}_exp2_layer14_numina_cot_lr5e-6"
if should_run 2; then
    echo "=========================================="
    echo "  [2/4] Layer 14 NuminaMath-CoT, LR=5e-6, epochs=8"
    echo "=========================================="
...
# --- Exp 3: Layer 0 only NuminaMath-CoT, LR=5e-5 ---
EXP3_NAME="${DATE}_exp3_layer0_numina_cot_lr5e-5"
if should_run 3; then
    echo "=========================================="
    echo "  [3/4] Layer 0 NuminaMath-CoT, LR=5e-5, epochs=8"
    echo "=========================================="
...
# --- Exp 4: Layer 27 only NuminaMath-CoT, LR=5e-5 ---
EXP4_NAME="${DATE}_exp4_layer27_numina_cot_lr5e-5"
if should_run 4; then
    echo "=========================================="
    echo "  [4/4] Layer 27 NuminaMath-CoT, LR=5e-5, epochs=8"
    echo "=========================================="

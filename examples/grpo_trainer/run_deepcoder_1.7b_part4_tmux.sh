#!/bin/bash
# ==============================================================================
# Part 4/4: Qwen3-1.7B-Base Layers 21-27 on DeepCoder (8K)
# ==============================================================================
# 7 experiments, epochs=2, max_response_length=8192, 8 GPUs
#
# Usage:
#   bash run_deepcoder_1.7b_part4_tmux.sh
#   bash run_deepcoder_1.7b_part4_tmux.sh --skip N
#   bash run_deepcoder_1.7b_part4_tmux.sh --only 1,3
#   bash run_deepcoder_1.7b_part4_tmux.sh --no-tmux
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="dcp4_$(date +%m%d_%H%M)"
...
should_run 1 && \
    run_train "${DATE}_exp1_layer21_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=21" && \
    echo "  [1/$TOTAL] Done." && echo ""

should_run 2 && \
    run_train "${DATE}_exp2_layer22_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=22" && \
    echo "  [2/$TOTAL] Done." && echo ""

should_run 3 && \
    run_train "${DATE}_exp3_layer23_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=23" && \
    echo "  [3/$TOTAL] Done." && echo ""

should_run 4 && \
    run_train "${DATE}_exp4_layer24_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=24" && \
    echo "  [4/$TOTAL] Done." && echo ""

should_run 5 && \
    run_train "${DATE}_exp5_layer25_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=25" && \
    echo "  [5/$TOTAL] Done." && echo ""

should_run 6 && \
    run_train "${DATE}_exp6_layer26_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=26" && \
    echo "  [6/$TOTAL] Done." && echo ""

should_run 7 && \
    run_train "${DATE}_exp7_layer27_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=27" && \

#!/bin/bash
# ==============================================================================
# Part 1/4: Qwen3-1.7B-Base Full RL + Layers 0-6 on DeepCoder (8K)
# ==============================================================================
# 8 experiments, epochs=2, max_response_length=8192, 8 GPUs
#
# Usage:
#   bash run_deepcoder_1.7b_part1_tmux.sh
#   bash run_deepcoder_1.7b_part1_tmux.sh --skip N
#   bash run_deepcoder_1.7b_part1_tmux.sh --only 1,3
#   bash run_deepcoder_1.7b_part1_tmux.sh --no-tmux
#
# Pre-req: data prepared at $DATA_DIR via:
...
DATA_DIR="$PROJ_DIR/data/deepcoder"
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="dcp1_$(date +%m%d_%H%M)"
...
DATE=$(date +%m%d_%H%M)
[[ ! -f "$DATA_DIR/train.parquet" ]] && echo "ERROR: $DATA_DIR/train.parquet not found.  Run examples/data_preprocess/deepcoder.py first." && exit 1
...
run_train() {
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.logger='["console","wandb"]' trainer.project_name=verl_grpo_deepcoder \
...
TOTAL=8
echo "============================================================"
echo "  Part 1/4: $MODEL_SHORT — Full RL + Layers 0-6 ($TOTAL exp)"
echo "  Data: $DATA_DIR | GPUs: $GPUS ($NGPUS) | epochs=2 | 8K"
echo "============================================================"

should_run 1 && run_train "${DATE}_exp1_full_${MODEL_SHORT}_deepcoder_lr1e-6" "$DATA_DIR" "1e-6" "" && echo "  [1/$TOTAL] Done." && echo ""

should_run 2 && \
    run_train "${DATE}_exp2_layer0_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=0" && \
    echo "  [2/$TOTAL] Done." && echo ""

should_run 3 && \
    run_train "${DATE}_exp3_layer1_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=1" && \
    echo "  [3/$TOTAL] Done." && echo ""

should_run 4 && \
    run_train "${DATE}_exp4_layer2_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=2" && \
    echo "  [4/$TOTAL] Done." && echo ""

should_run 5 && \
    run_train "${DATE}_exp5_layer3_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=3" && \
    echo "  [5/$TOTAL] Done." && echo ""

should_run 6 && \
    run_train "${DATE}_exp6_layer4_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=4" && \
    echo "  [6/$TOTAL] Done." && echo ""

should_run 7 && \
    run_train "${DATE}_exp7_layer5_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=5" && \
    echo "  [7/$TOTAL] Done." && echo ""

should_run 8 && \
    run_train "${DATE}_exp8_layer6_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=6" && \

#!/bin/bash
# Train on APPS dataset with layer-selective freezing.
#
# Usage:
#   bash examples/grpo_trainer/run_apps.sh
#   bash examples/grpo_trainer/run_apps.sh --train-layer-ids 13 --lr 1e-5
...
if [[ -z "$TMUX" ]] && [[ "$NO_TMUX" == "false" ]]; then
    ...
    tmux new-session -d -s "$TMUX_SESSION" \
        "source /code/hongpaul-sandbox/cuda/miniconda3/bin/activate && \
         ...
         cd $PROJ_DIR && \
         bash $SCRIPT_DIR/run_apps.sh --no-tmux $ARGS; \
         exec bash"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=verl_grpo_apps \

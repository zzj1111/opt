#!/bin/bash
# ==============================================================================
# Part 1/2: Qwen3-4B-Base Full RL + Layers 0-17 on NuminaMath-CoT
# ==============================================================================
#
# Experiments (19 total):
#   1.  Full RL Qwen3-4B-Base,   LR=1e-6
#   2-19. Layer 0-17,            LR=5e-6
#
# batch=512, minibatch=128, microbatch=8, epochs=2, max_response_length=3072, 8 GPUs
#
# Usage:
#   bash run_4b_base_part1_tmux.sh
#   bash run_4b_base_part1_tmux.sh --skip 1        # skip full RL
#   bash run_4b_base_part1_tmux.sh --only 1,5
#   bash run_4b_base_part1_tmux.sh --no-tmux
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="4b_p1_$(date +%m%d_%H%M)"
...
DUMMY_RUN_NAME="dummy_4b_p1_$(hostname)_$(date +%m%d_%H%M)" python3 "$SCRIPT_DIR/dummy_gpu_hold.py"

#!/bin/bash
# ==============================================================================
# Part 2/4: Qwen3-1.7B-Base Layers 7-13 on DeepCoder (8K)
# ==============================================================================
# 7 experiments, epochs=2, max_response_length=8192, 8 GPUs
#
# Usage:
#   bash run_deepcoder_1.7b_part2_tmux.sh
#   bash run_deepcoder_1.7b_part2_tmux.sh --skip N
#   bash run_deepcoder_1.7b_part2_tmux.sh --only 1,3
#   bash run_deepcoder_1.7b_part2_tmux.sh --no-tmux
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="dcp2_$(date +%m%d_%H%M)"
...
TOTAL=7
echo "============================================================"
echo "  Part 2/4: $MODEL_SHORT — Layers 7-13 ($TOTAL exp)"
echo "  Data: $DATA_DIR | GPUs: $GPUS ($NGPUS) | epochs=2 | 8K"
echo "============================================================"

should_run 1 && \
    run_train "${DATE}_exp1_layer7_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=7" && \
    echo "  [1/$TOTAL] Done." && echo ""

should_run 2 && \
    run_train "${DATE}_exp2_layer8_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=8" && \
    echo "  [2/$TOTAL] Done." && echo ""

should_run 3 && \
    run_train "${DATE}_exp3_layer9_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=9" && \
    echo "  [3/$TOTAL] Done." && echo ""

should_run 4 && \
    run_train "${DATE}_exp4_layer10_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=10" && \
    echo "  [4/$TOTAL] Done." && echo ""

should_run 5 && \
    run_train "${DATE}_exp5_layer11_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=11" && \
    echo "  [5/$TOTAL] Done." && echo ""

should_run 6 && \
    run_train "${DATE}_exp6_layer12_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=12" && \
    echo "  [6/$TOTAL] Done." && echo ""

should_run 7 && \
    run_train "${DATE}_exp7_layer13_${MODEL_SHORT}_deepcoder_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=13" && \

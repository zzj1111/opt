#!/bin/bash
# ==============================================================================
# 5 Experiments (Part 1): Qwen3-1.7B single-layer RL training with MATH (with tmux)
# ==============================================================================
#
# Experiments:
#   1. Layer 0 only MATH,      LR=5e-5
#   2. Layer 3 only MATH,      LR=5e-5
#   3. Layer 6 only MATH,      LR=5e-5
#   4. Layer 9 only MATH,      LR=5e-5
#   5. Layer 12 only MATH,     LR=5e-5
#
...
#
# Usage:
#   bash run_math_layers_part1_tmux.sh                          # Run all 5
#   bash run_math_layers_part1_tmux.sh --gpus 0,1,2,3,4,5,6,7  # Use specific GPUs
#   bash run_math_layers_part1_tmux.sh --skip 2                 # Skip first 2
#   bash run_math_layers_part1_tmux.sh --only 1,3               # Run only 1 and 3
#   bash run_math_layers_part1_tmux.sh --no-tmux                # Skip tmux auto-launch
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="math_layers_p1_$(date +%m%d_%H%M)"
    ...
    tmux new-session -d -s "$TMUX_SESSION" \
        "source $CONDA_INIT && \
         ...
         cd $PROJ_DIR && \
         bash $SCRIPT_DIR/run_math_layers_part1_tmux.sh $FULL_ARGS; \
         exec bash"
...
# ========== Run Experiments ==========
echo "============================================================"
echo "  5 Experiments (Part 1): Single-Layer RL on MATH"
echo "  Layers: 0, 3, 6, 9, 12 | LR=5e-5 | epochs=8"
...
# --- Exp 1: Layer 0 ---
EXP1_NAME="${DATE}_exp1_layer0_math_lr5e-5"
if should_run 1; then
    echo "=========================================="
    echo "  [1/5] Layer 0 MATH, LR=5e-5"
    echo "=========================================="
...
# --- Exp 2: Layer 3 ---
EXP2_NAME="${DATE}_exp2_layer3_math_lr5e-5"
if should_run 2; then
    echo "=========================================="
    echo "  [2/5] Layer 3 MATH, LR=5e-5"
    echo "=========================================="
...
# --- Exp 3: Layer 6 ---
EXP3_NAME="${DATE}_exp3_layer6_math_lr5e-5"
if should_run 3; then
    echo "=========================================="
    echo "  [3/5] Layer 6 MATH, LR=5e-5"
    echo "=========================================="
...
# --- Exp 4: Layer 9 ---
EXP4_NAME="${DATE}_exp4_layer9_math_lr5e-5"
if should_run 4; then
    echo "=========================================="
    echo "  [4/5] Layer 9 MATH, LR=5e-5"
    echo "=========================================="
...
# --- Exp 5: Layer 12 ---
EXP5_NAME="${DATE}_exp5_layer12_math_lr5e-5"
if should_run 5; then
    echo "=========================================="
    echo "  [5/5] Layer 12 MATH, LR=5e-5"
    echo "=========================================="
...
echo "============================================================"
DUMMY_RUN_NAME="dummy_math_layers_p1_$(hostname)_$(date +%m%d_%H%M)" \

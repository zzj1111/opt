#!/bin/bash
# ==============================================================================
# 5 Experiments: Qwen3-1.7B single-layer RL training with NuminaMath-CoT (with tmux)
# ==============================================================================
#
# Experiments:
#   1. Layer 3 only NuminaMath-CoT,      LR=5e-5
#   2. Layer 6 only NuminaMath-CoT,      LR=5e-5
#   3. Layer 9 only NuminaMath-CoT,      LR=1e-5
#   4. Layer 12 only NuminaMath-CoT,     LR=5e-6
#   5. Layer 15 only NuminaMath-CoT,     LR=5e-6
#
# All: batch=256, minibatch=64, microbatch=4, epochs=2, max_response_length=8192
# Model: Qwen3-1.7B, 8 GPUs, saves only last-step checkpoint in HuggingFace format.
#
# Usage:
#   bash run_numina_cot_5exp_layers_tmux.sh                          # Run all 5
#   bash run_numina_cot_5exp_layers_tmux.sh --gpus 0,1,2,3,4,5,6,7  # Use specific GPUs
#   bash run_numina_cot_5exp_layers_tmux.sh --skip 2                 # Skip first 2
#   bash run_numina_cot_5exp_layers_tmux.sh --only 1,3               # Run only 1 and 3
#   bash run_numina_cot_5exp_layers_tmux.sh --no-tmux                # Skip tmux auto-launch
#
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="numina_5exp_layers_$(date +%m%d_%H%M)"
    ...
    tmux new-session -d -s "$TMUX_SESSION" \
        "source $CONDA_INIT && \
         ...
         cd $PROJ_DIR && \
         bash $SCRIPT_DIR/run_numina_cot_5exp_layers_tmux.sh $FULL_ARGS; \
         exec bash"
...
# ========== Run Experiments ==========
echo "============================================================"
echo "  5 Experiments: Qwen3-1.7B Single-Layer RL on NuminaMath-CoT"
...
# --- Exp 1: Layer 3 only NuminaMath-CoT, LR=5e-5 ---
EXP1_NAME="${DATE}_exp1_layer3_numina_cot_lr5e-5"
if should_run 1; then
    echo "=========================================="
    echo "  [1/5] Layer 3 NuminaMath-CoT, LR=5e-5"
    echo "=========================================="
...
# --- Exp 2: Layer 6 only NuminaMath-CoT, LR=5e-5 ---
EXP2_NAME="${DATE}_exp2_layer6_numina_cot_lr5e-5"
if should_run 2; then
    echo "=========================================="
    echo "  [2/5] Layer 6 NuminaMath-CoT, LR=5e-5"
    echo "=========================================="
...
# --- Exp 3: Layer 9 only NuminaMath-CoT, LR=1e-5 ---
EXP3_NAME="${DATE}_exp3_layer9_numina_cot_lr1e-5"
if should_run 3; then
    echo "=========================================="
    echo "  [3/5] Layer 9 NuminaMath-CoT, LR=1e-5"
    echo "=========================================="
...
# --- Exp 4: Layer 12 only NuminaMath-CoT, LR=5e-6 ---
EXP4_NAME="${DATE}_exp4_layer12_numina_cot_lr5e-6"
if should_run 4; then
    echo "=========================================="
    echo "  [4/5] Layer 12 NuminaMath-CoT, LR=5e-6"
    echo "=========================================="
...
# --- Exp 5: Layer 15 only NuminaMath-CoT, LR=5e-6 ---
EXP5_NAME="${DATE}_exp5_layer15_numina_cot_lr5e-6"
if should_run 5; then
    echo "=========================================="
    echo "  [5/5] Layer 15 NuminaMath-CoT, LR=5e-6"
    echo "=========================================="
...
echo "============================================================"
DUMMY_RUN_NAME="dummy_numina_5exp_layers_$(hostname)_$(date +%m%d_%H%M)" \

#!/bin/bash
# ==============================================================================
# Part 3/4: Qwen3-1.7B-Base Layers 14-20 on DeepScaleR (8K)
# ==============================================================================
# 7 experiments, epochs=2, max_response_length=8192, 8 GPUs
#
# Usage:
#   bash run_deepscaler_1.7b_part3_tmux.sh
#   bash run_deepscaler_1.7b_part3_tmux.sh --skip N
#   bash run_deepscaler_1.7b_part3_tmux.sh --only 1,3
#   bash run_deepscaler_1.7b_part3_tmux.sh --no-tmux
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="dsr_p3_$(date +%m%d_%H%M)"
...
should_run 1 && \
    run_train "${DATE}_exp1_layer14_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=14" && \
    echo "  [1/$TOTAL] Done." && echo ""

should_run 2 && \
    run_train "${DATE}_exp2_layer15_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=15" && \
    echo "  [2/$TOTAL] Done." && echo ""

should_run 3 && \
    run_train "${DATE}_exp3_layer16_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=16" && \
    echo "  [3/$TOTAL] Done." && echo ""

should_run 4 && \
    run_train "${DATE}_exp4_layer17_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=17" && \
    echo "  [4/$TOTAL] Done." && echo ""

should_run 5 && \
    run_train "${DATE}_exp5_layer18_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=18" && \
    echo "  [5/$TOTAL] Done." && echo ""

should_run 6 && \
    run_train "${DATE}_exp6_layer19_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=19" && \
    echo "  [6/$TOTAL] Done." && echo ""

should_run 7 && \
    run_train "${DATE}_exp7_layer20_${MODEL_SHORT}_deepscaler_lr5e-6" "$DATA_DIR" "5e-6" "+actor_rollout_ref.actor.train_layer_ids=20" && \
    echo "  [7/$TOTAL] Done." && echo ""

echo ""; echo "  Part 3/4 complete!"
DUMMY_RUN_NAME="dummy_dsr_p3_$(hostname)_$(date +%m%d_%H%M)" python3 "$SCRIPT_DIR/dummy_gpu_hold.py"

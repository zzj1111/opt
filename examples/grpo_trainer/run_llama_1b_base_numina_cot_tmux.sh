#!/bin/bash
# ==============================================================================
# Llama-3.2-1B Full RL + every-3rd-layer ablation on NuminaMath-CoT
# ==============================================================================
#
# Llama-3.2-1B has 16 layers (0..15). To keep the sweep manageable we sample
# every third layer: 0, 3, 6, 9, 12, 15 (6 indices) plus the full-RL run.
#
# Experiments (7 total):
#   1.  Full RL Llama-3.2-1B,    LR=1e-6
#   2-7. Layers 0/3/6/9/12/15,   LR=5e-6
#
# batch=512, minibatch=128, microbatch=8, epochs=2, max_response_length=3072, 8 GPUs
#
# Usage:
#   bash run_llama_1b_base_numina_cot_tmux.sh
#   bash run_llama_1b_base_numina_cot_tmux.sh --skip 1        # skip full RL
#   bash run_llama_1b_base_numina_cot_tmux.sh --only 1,3
#   bash run_llama_1b_base_numina_cot_tmux.sh --no-tmux
#   bash run_llama_1b_base_numina_cot_tmux.sh --model /local/path/to/Llama-3.2-1B
...
# Layer indices to ablate (every 3rd of 16). Override with --layers.
LAYERS_TO_RUN="0 3 6 9 12 15"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --layers)     LAYERS_TO_RUN="$2"; shift 2 ;;
        --skip)       SKIP="$2"; shift 2 ;;
        --only)       ONLY="$2"; shift 2 ;;
...
if [[ -z "${TMUX:-}" ]] && [[ "$NO_TMUX" == "false" ]]; then
    TMUX_SESSION="llama1b_$(date +%m%d_%H%M)"
    FULL_ARGS="--no-tmux --gpus $(printf '%q' "$GPUS") --model $(printf '%q' "$MODEL") --ckpt-root $(printf '%q' "$CKPT_ROOT") --data-dir $(printf '%q' "$DATA_DIR") --layers $(printf '%q' "$LAYERS_TO_RUN")"
...
# Build the list of layer indices and total experiment count
read -ra LAYER_ARR <<< "$LAYERS_TO_RUN"
TOTAL=$((${#LAYER_ARR[@]} + 1))     # +1 for full RL

echo "============================================================"
echo "  $MODEL_SHORT — Full RL + Layers [${LAYERS_TO_RUN}]  ($TOTAL exp)"
echo "  Data: $DATA_DIR | GPUs: $GPUS ($NGPUS) | epochs=2 | resp=3k"
echo "============================================================"

# Exp 1: Full RL
should_run 1 && run_train "${DATE}_exp1_full_${MODEL_SHORT}_numina_cot_lr1e-6" "$DATA_DIR" "1e-6" "" && echo "  [1/$TOTAL] Done." && echo ""

# Exp 2..N: Layer ablations
EXP_NUM=2
...
echo ""; echo "  Llama-1B sweep complete!"
DUMMY_RUN_NAME="dummy_llama1b_$(hostname)_$(date +%m%d_%H%M)" python3 "$SCRIPT_DIR/dummy_gpu_hold.py"

#!/bin/bash
# Train on TACO (Easy+Medium) dataset with layer-selective freezing.
#
# Usage:
#   bash examples/grpo_trainer/run_taco.sh
#   bash examples/grpo_trainer/run_taco.sh --train-layer-ids 13 --lr 1e-5
#   bash examples/grpo_trainer/run_taco.sh --model Qwen/Qwen3-1.7B --gpus 0,1,2,3,4,5,6,7
...
EXP_NAME="taco_${DATE}_${MODEL_SHORT}_lr${LR}"
if [[ -n "$ROUND" ]]; then EXP_NAME="taco_${DATE}_r${ROUND}_${MODEL_SHORT}_lr${LR}"; fi
...
if [[ -z "$TMUX" ]] && [[ "$NO_TMUX" == "false" ]]; then
    ...
    tmux new-session -d -s "$TMUX_SESSION" \
        "source /code/hongpaul-sandbox/cuda/miniconda3/bin/activate && \
         ...
         cd $PROJ_DIR && \
         bash $SCRIPT_DIR/run_taco.sh --no-tmux $ARGS; \
         exec bash"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name=verl_grpo_taco \
